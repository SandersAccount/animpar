...
export function applyEnterExitEffects(shape: any, currentFrame: number): any {
  let result = { ...shape }

  // Apply enter animation first
  if (shape.animation?.enter) {
    result = applyEnterAnimation(result, currentFrame)
  }

  // Then apply exit animation
  if (shape.animation?.exit) {
    result = applyExitAnimation(result, currentFrame)
  }
