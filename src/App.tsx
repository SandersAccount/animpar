...
import { Shape, ShapeType, AnimationType, FocalPoint } from './types'
...
const App: React.FC = () => {
  ...
  const handleFocalPointUpdate = (newFocalPoint: FocalPoint) => {
    // Add or update keyframe for focal point
    const existingKeyframe = focalPoint.keyframes.find(kf => kf.frame === currentFrame)
    let updatedKeyframes = [...focalPoint.keyframes]

    if (existingKeyframe) {
      updatedKeyframes = updatedKeyframes.map(kf => 
        kf.frame === currentFrame ? { ...kf, x: newFocalPoint.x, y: newFocalPoint.y } : kf
      )
    } else {
      ...
      <div className="flex-1 flex flex-col p-4">
        <div className="flex-1">
          <Canvas
            ...
            updateFocalPoint={handleFocalPointUpdate}
          />
        </div>
        <Timeline
          selectedShape={selectedShape}
